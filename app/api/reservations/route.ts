...
export async function GET(request: NextRequest) {
  try {
    ...
    // 필터링 적용
    let filteredReservations = [...extendedMockReservations]
    ...
    if (customerId) {
      filteredReservations = filteredReservations.filter((reservation) => reservation.customerName.includes(customerId))
    }

    if (licensePlate) {
      filteredReservations = filteredReservations.filter((reservation) =>
        reservation.licensePlate.includes(licensePlate),
      )
    }

    if (assignedTo) {
      filteredReservations = filteredReservations.filter((reservation) => reservation.assignedTo === assignedTo)
    }
